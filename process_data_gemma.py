...
OUTPUT_DIR = "./processed_data_trace"
...
def linearize_trace_gemma(example):
    conversations = example.get('conversations', [])
    ...
    if steps:
        ...
        for i, step in enumerate(steps[:-1]):
            ...
            if role == 'assistant':
                ...
                # Smart Injection: Only inject tags if the tool actually failed
...
def main():
    ...
    print("Linearizing traces (Main Method)...")
    processed_samples = []
    tool_map = {} 
    ...
    for sample in tqdm(raw_dataset):
        ...
        try:
            linearized = linearize_trace_gemma(sample)
            if linearized:
    ...
    print(f"Saved Main Data: {len(train_samples)} Train | {len(val_samples)} Val")
